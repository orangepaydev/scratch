...
|-----------------------------|-----------------------------------------------------------------------|-----------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------|----------------------------------|
| CA Certificate              | Used establish trust                                                  | [Engine]Validate the Cert chain                     | [Vault]pki/rootCA${YYYYMMDD} <br> [SSM]/app-workflow-v5/license/CACert[1/2]                                                      | rootCA${YYYYMMDD}                |
...
| Server Certificate          | Used to encrypt the license payload                                   | [Portal]Encrypt the License Payload                 | [Vault]pki_int/${serialNumber} <br> [SSM]/portal-license-v1/license/ServerPublicCert                                             | app-workflow-v5.V5.${YYYYMMDD}   |
...
| Portal Certificate          | Used by the App Server to verify the signature of the license payload | [Engine]Verify the signature of the license payload | [Vault]pki_int/${serialNumber} <br> [SSM]/portal-license-v1/license/PortalPublicCert (contains the cert + intermediate CA chain) | portal-license-v1.V1.${YYYYMMDD} |
| Portal Private Key          | Use by the Portal to sign the license payload                         | [Portal]Sign the license payload                    | [Vault]kv_v1/${serialNumber} <br> [SSM]/portal-license-v1/license/PortalEncKey                                                   |                                  |
...
Only the Server require the current and inactive certificate / key. The Portal will always use the latest certificate /
key.
